...
<script>
  ...
  function texi_in_disable_hazard_selector_from_level(level){
	  ...
	  for(var cur_level=level+1 ; cur_level <= 5 ; cur_level++){
		  ...
		  var selectItem = document.getElementById('id_taxi_in_level_'+cur_level+'_selector');
  ...
  function taxi_in_enable_description_of_new_hazard(val){
	  if (val == 'y'){
		  $('#id_taxi_in_description_of_new_hazard').removeAttr("disabled");
	  }else if( val == 'n'){
		  $('#id_taxi_in_description_of_new_hazard').attr("disabled", true);
	  }
  ...
  function texi_in_read_hazard_item_list_top(callback){	
		$.ajax({
			...
			success: function(msg){
				...
				var selectItem = document.getElementById('id_taxi_in_level_1_selector');
				//Remove all Items
				for(var count = 0 ; count < selectItem.options.length ; count++)
				{
					selectItem.options[count] = null;
  ...
  function texi_in_read_hazard_item_list_children(id, level, callback){ 
		$.ajax({
			...
			success: function(msg){
				...
				Result = jQuery.trim(Result);
				var platforms = Result.split(",");
				var selectItem = document.getElementById('id_taxi_in_level_'+(parseInt(level, 10)+1)+'_selector');
				//Remove all Items
				for(var count = 0 ; count < selectItem.options.length ; count++)
				{
					selectItem.options[count] = null;
  ...
  {
	  var return_str = '<a id="id_taxi_in_seq_'+cellvalue+'_edit_hazard" href="#">Edit</a><a id="id_taxi_in_seq_'+cellvalue+'_delete_hazard" href="#">Delete</a>';
		return_str += '<script>';
		return_str += '$("#id_taxi_in_seq_'+cellvalue+'_edit_hazard").button().click(function( event ) {'
		return_str += '    	event.preventDefault();';
		return_str += '});';
		return_str += '$("#id_taxi_in_seq_'+cellvalue+'_delete_hazard").button().click(function( event ) {'
		return_str += '    	event.preventDefault();';
		return_str += '});';
		return_str += '</scr'+'ipt>';
	  	return return_str;
	  ...
	  jQuery("#id_taxi_in_hazardListTable").jqGrid({
	  	...
	  	datatype: "xml", 
	     	...
	     	colModel:[
	     	    		...
	     	    		{name:'fn'			,index:'fn'				,width:95	,align:"center", formatter:texi_in_fnFormatter	}		
	     	    	],
	     	//shrinkToFit:true,
	     	//altRows:true,
	     	hoverrows:false,
	  	...
	  	loadComplete: function(){ 
	  			...
	  			//jQuery("#id_taxi_in_hazardListTable").setRowData(ids[i],{detail:detailHtml});				
...
<body>
<h2 class="ui-widget-header">TAXI-IN INFORMATION</h2>
...
<tr>
	...
	<td><select id="id_taxi_in_level_4_selector" onchange="texi_in_read_hazard_item_list_children(this.value,4);" name="method" class="hazard_item_selector">
	</select> </td>
</tr>
<tr>
	<td class="leftmost_label">Level5: </td>
	<td><select id="id_taxi_in_level_5_selector" name="method" class="hazard_item_selector">
	</select> </td>
</tr>
</tbody>
</table>
...
<tr>
	...
	<td> <input type="text" id="id_taxi_inspecific_items" class="hazard_item_text_input" /> </td>
</tr>
</tbody>
</table>
<table>
<tbody>
<tr>
	<td class="leftmost_label"></td><td align="right" width="723px;"><a id="id_taxi_insave_hazard" href="#">Done</a> <a id="id_taxi_inmore_hazards" href="#">More Hazards?</a></td>
</tr>
</tbody>
</table>
...
<tr>
	...
	<td align="center" width="723px;" ><table id="id_taxi_in_hazardListTable" class="scroll" cellpadding="0" cellspacing="0"></table>
...
<tr>
	...
	<td><textarea rows="4" cols="100" id="id_taxi_in_description_of_new_hazard" disabled="disabled"></textarea></td>
</tr>
</tbody>
</table>
...
<tr>
	...
	<td><textarea rows="4" cols="100" id="id_taxi_in_narrative"></textarea></td>
</tr>
<tr>
	<td class="leftmost_label">Recommendation</td>
	<td><textarea rows="4" cols="100" id="id_taxi_in_recommendation"></textarea></td>
</tr>
</tbody>
</table>
<table width="100%">
