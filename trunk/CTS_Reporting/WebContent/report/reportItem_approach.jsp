...
<script>
  $(function() {
   $("#id_approachmore_hazards")
   .button()
   .click(function( event ) {
	 fn_approach_reset_hazard_identification();
     event.preventDefault();
   });
   $("#id_approachsave_hazard")
   .button()
   .click(function( event ) {
     event.preventDefault();
   });
   
   $("#id_approach_save_btn")
   .button()
   .click(function( event ) {
     event.preventDefault();
   });
   $("#id_approach_previous_btn")
   .button()
   .click(function( event ) {
     changeTab(6);
     event.preventDefault();
   });
   $("#id_approach_next_btn")
   .button()
   .click(function( event ) {
	 changeTab(8);
     event.preventDefault();
   ...
   fn_approach_read_hazard_item_list_top();
   fn_approach_disable_hazard_selector_from_level(1);
   
   fn_approach_load_hazard_item();
  });
  
  function fn_approach_reset_hazard_identification(){
	  fn_approach_read_hazard_item_list_top();
	  fn_approach_disable_hazard_selector_from_level(1);
  }
  
  function fn_approach_disable_hazard_selector_from_level(level){
	  //alert(level);
	  for(var cur_level=level+1 ; cur_level <= 5 ; cur_level++){
		  $('#id_approach_level_'+cur_level+'_selector').attr("disabled", true);
		  var selectItem = document.getElementById('id_approach_level_'+cur_level+'_selector');
  ...
  function fn_approach_enable_hazard_selector(level){
	  $('#id_approach_level_'+level+'_selector').removeAttr("disabled");
  }	
  
  function fn_approach_approach_enable_description_of_new_hazard(val){
	  if (val == 'y'){
		  $('#id_approach_description_of_new_hazard').removeAttr("disabled");
	  }else if( val == 'n'){
		  $('#id_approach_description_of_new_hazard').attr("disabled", true);
	  }
  ...
  function fn_approach_read_hazard_item_list_top(callback){	
		$.ajax({
			type: "POST",
			...
			success: function(msg){
				...
				var selectItem = document.getElementById('id_approach_level_1_selector');
				//Remove all Items
				for(var count = 0 ; count < selectItem.options.length ; count++)
				{
					selectItem.options[count] = null;
  ...
  function fn_approach_read_hazard_item_list_children(id, level, callback){ 
		$.ajax({
			type: "POST",
			...
			success: function(msg){
				...
				Result = jQuery.trim(Result);
				var platforms = Result.split(",");
				var selectItem = document.getElementById('id_approach_level_'+(parseInt(level, 10)+1)+'_selector');
				//Remove all Items
				for(var count = 0 ; count < selectItem.options.length ; count++)
				{
					selectItem.options[count] = null;
				...
				fn_approach_disable_hazard_selector_from_level(parseInt(level, 10)+1);
				fn_approach_enable_hazard_selector(parseInt(level, 10)+1);
			}
		});
  }
  
  function fn_approach_dateFormatter( cellvalue, options, rowObject )
  {
  ...
  function fn_approach_load_hazard_item(){
	  ...
	  jQuery("#id_approach_hazardListTable").jqGrid({
	  	...
	  	loadComplete: function(){ 
	  			...
	  			//jQuery("#id_approach_hazardListTable").setRowData(ids[i],{detail:detailHtml});				
...
<body>
<h2 class="ui-widget-header">APPROACH INFORMATION</h2>
...
<tr>
	...
	<td><select id="id_approach_level_1_selector" onchange="fn_approach_read_hazard_item_list_children(this.value,1);" name="method" class="hazard_item_selector">		
	</select> </td>
</tr>
<tr>
	<td class="leftmost_label">Level2: </td>
	<td><select id="id_approach_level_2_selector" onchange="fn_approach_read_hazard_item_list_children(this.value,2);" name="method" class="hazard_item_selector">
	</select> </td>
</tr>
<tr>
	<td class="leftmost_label">Level3: </td>
	<td><select id="id_approach_level_3_selector" onchange="fn_approach_read_hazard_item_list_children(this.value,3);" name="method" class="hazard_item_selector">
	</select> </td>
</tr>
<tr>
...
<tr>
	...
	<td><select id="id_approach_level_5_selector" name="method" class="hazard_item_selector">
	</select> </td>
</tr>
</tbody>
</table>
...
<tr>
	...
	<td> <input type="text" id="id_approachspecific_items" class="hazard_item_text_input" /> </td>
</tr>
</tbody>
</table>
<table>
<tbody>
<tr>
	<td class="leftmost_label"></td><td align="right" width="723px;"><a id="id_approachsave_hazard" href="#">Done</a> <a id="id_approachmore_hazards" href="#">More Hazards?</a></td>
</tr>
</tbody>
</table>
...
<tr>
	...
	<td align="center" width="723px;" ><table id="id_approach_hazardListTable" class="scroll" cellpadding="0" cellspacing="0"></table>
...
<tr>
	...
	<td> <input type="radio" name="new_hazard" id="approach_new_hazard_yes" onchange="fn_approach_approach_enable_description_of_new_hazard('n');" value="n" checked="checked"/></td>
	<td style="width:30px;text-align:right;">Yes</td>
	<td> <input type="radio"name="new_hazard" id="approach_new_hazard_no" onchange="fn_approach_approach_enable_description_of_new_hazard('y');" value="y"/></td>
</tr>
</tbody>
</table>
<table>
...
<tr>
	...
	<td><textarea rows="4" cols="100" id="id_approach_description_of_new_hazard" disabled="disabled"></textarea></td>
</tr>
</tbody>
</table>
...
<tr>
	...
	<td><textarea rows="4" cols="100" id="id_approach_narrative"></textarea></td>
</tr>
<tr>
	<td class="leftmost_label">Recommendation</td>
	<td><textarea rows="4" cols="100" id="id_approach_recommendation"></textarea></td>
</tr>
</tbody>
</table>
<table width="100%">
<tbody>
<tr>
	<td align="right"><a id="id_approach_save_btn" href="#">Save</a><a id="id_approach_previous_btn" href="#">Previous</a><a id="id_approach_next_btn" href="#">Next</a></td>
</tr>
</tbody>
</table>
</div>
