...
import ac.kaist.analysis.utils.DateUtil;
		...
		for(int i = 0 ; i < sheet.getRows() ; i++){
			...
			for(int j = 0 ; j < sheet.getColumns() ; j++){
				...
				//Otherwise normal content
					...
					//if(headers.get(j).equalsIgnoreCase("��¥"))
...
//		}
	...
	public Map<String, Integer> getDateColumnSumByCondition(String dateHeader, String dataHeader, int dateType, String condHeader, String condValue){
		...
		for(int i = 0 ; i < contents.size(); i++){
			//sum += Integer.parseInt(contents.get(i).get(columnIndex));
			String dateStr = DateUtil.convertJodaDateFormat(contents.get(i).get(dateIndex));
			inputDateMap.put(new LocalDate(dateStr), 0);
		}
		
		Map<String, Integer> dateMap = DateUtil.getDateArray(inputDateMap, dateType);
		for(int i = 0 ; i < contents.size(); i++){
			if(condHeader != null){
			...
			String dateStr = DateUtil.convertJodaDateFormat(contents.get(i).get(dateIndex));
			dateStr = DateUtil.cutDateStr(dateStr, dateType);
			Integer data_val = Integer.parseInt(contents.get(i).get(dataIndex));
			Integer cur_val = dateMap.get(dateStr);
			dateMap.put(dateStr, cur_val + data_val);
		}
	...
	public Map<String, Float> getDateColumnAverageByCondition(String dateHeader, String dataHeader, int dateType, String condHeader, String condValue){
		int dateIndex = -1;
		int dataIndex = -1;
		int condIndex = -1;
		for(int i = 0 ; i < headers.size(); i++){
		...
		Map<String, Float> dateMapAverage = new TreeMap<String, Float>();
		for(int i = 0 ; i < contents.size(); i++){
			//sum += Integer.parseInt(contents.get(i).get(columnIndex));
			String dateStr = DateUtil.convertJodaDateFormat(contents.get(i).get(dateIndex));
			inputDateMap.put(new LocalDate(dateStr), 0);
			countMap.put(DateUtil.cutDateStr(dateStr, dateType), 0);
		}
		...
		for(String key : keySet){
			...
			if (cur_count == null)
				dateMapAverage.put(key, 0.0f);
			else if (cur_count == 0)
				dateMapAverage.put(key, 0.0f);
			else{
				float val = cur_val / (float)cur_count;
				float val_a = Float.parseFloat(String.format("%.2f" , val));
				dateMapAverage.put(key, val_a);
			}
		}
	...
	public Vector getColumnStringContents(String dataHeader){
		Vector<String> typeList = new Vector<String>();
		int dataIndex = getColumnIndex(dataHeader);
		
		for(int i = 0 ; i < contents.size(); i++){
			if (contents.get(i).size() > dataIndex){
				String code = contents.get(i).get(dataIndex);
				typeList.add(code);
			}
		}
	...
	public Vector getColumnIntegerContents(String dataHeader){
		Vector<String> sContent = getColumnStringContents(dataHeader);
		Vector<Integer> iContent = new Vector<Integer>();
		for( String a : sContent)
			iContent.add(Integer.parseInt(a));
		return iContent;
	}
	
	public Vector getColumnFloatContents(String dataHeader){
		Vector<String> sContent = getColumnStringContents(dataHeader);
		Vector<Float> fContent = new Vector<Float>();
		for( String a : sContent)
			fContent.add(Float.parseFloat(a));
		return fContent;
	}
	...
	public Map<String, Map<String, Float> > getEffectiveness(String dateHeader, String initialRiskHeader, String residualRiskHeader, String actionHeader){
		...
		Map<String, Map<String, Float> > nreffectiveness = new TreeMap<String, Map<String, Float> >();
		for(String action_code : codes){
			Map<String, Float> effectivenessMap = new TreeMap<String, Float>();
			
			Map<String, Float> averageInitialRiskByAction = this.getDateColumnAverageByCondition(dateHeader, initialRiskHeader, DateUtil.dateTypeMonth, actionHeader, action_code);
			Map<String, Float> averageResidualRiskByAction = this.getDateColumnAverageByCondition(dateHeader, residualRiskHeader, DateUtil.dateTypeMonth, actionHeader, action_code);
			//System.out.println("averageInitialRiskByAction : "+averageInitialRiskByAction);
			//System.out.println("averageResidualRiskByAction : "+averageResidualRiskByAction);
			Set<String> monthList = averageInitialRiskByAction.keySet();
			for(String month : monthList){
				Float initialRisk = averageInitialRiskByAction.get(month);
				Float ResidualRisk = averageResidualRiskByAction.get(month);
				Float effectiveness = initialRisk - ResidualRisk;
				//System.out.println("initialRisk: "+initialRisk +", ResidualRisk:"+ResidualRisk+", effectiveness:"+effectiveness);
