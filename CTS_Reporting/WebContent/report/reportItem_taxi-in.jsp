...
<script>
  $(function() {
	  $("#id_report_taxi_in_attach_file_btn")
   ...
   $("#id_report_taxi_in_hazard_title").example('ex) Pax door impated airbridege while opening');
   $("#id_report_taxi_in_hazard_time").timepicker();
  
   
   texi_out_load_hazard_item();
   
   CKEDITOR.replace( 'report_taxi_in_narrative_ckeditor' );
   CKEDITOR.replace( 'report_taxi_in_recommendation_ckeditor' );
  });
  ...
  {
	var return_str = '<a id="id_taxi_in_seq_'+cellvalue+'_delete_hazard" href="#">${lang.getStringDelete()}</a>';
	return_str += '<script>';
	return_str += '$("#id_taxi_in_seq_'+cellvalue+'_delete_hazard").button({icons: {secondary: "ui-icon-trash" } }).click(function( event ) {'
  ...
  function texi_out_load_hazard_item(){
	  ...
	  jQuery("#id_taxi_in_attached_file_list_table").jqGrid({
	  	...
	  	loadComplete: function(){ 
	  			...
	  			//jQuery("#id_taxi_in_attached_file_list_table").setRowData(ids[i],{detail:detailHtml});				
...
<legend class="l1_fieldset_legend">TAXI-IN ${lang.getStringInformation()}</legend>

<table width="100%" cellpadding="0" cellspacing="0">
	<tbody>
		<tr>
			<td width="90">${lang.getStringHazardTitle()}</td>
			<td><input type="text" style="width:100%" name="" id="id_report_taxi_in_hazard_title" value=""/></td>
		</tr>
		<tr>
			...
			<td>
				...
				<tbody>
					<tr>
						<td width="10%"><input type="text" style="width:100%; height:12px;" name="" id="id_report_taxi_in_hazard_time" value=""/><td>
						<td width="5%" align="right"> <input type="radio" name="new_hazard" id="taxi_in_new_hazard_no" onchange="taxi_in_enable_description_of_new_hazard('n');" value="n" checked="checked"/></td>
						<td width="5%" align="left">Local</td>
						<td width="5%" align="right"> <input type="radio"name="new_hazard" id="taxi_in_new_hazard_yes" onchange="taxi_in_enable_description_of_new_hazard('y');" value="y"/></td>
		...
		<tr>
			...
			<fieldset class="l2_fieldset">
				...
				<textarea name="report_taxi_in_narrative_ckeditor" rows="3" style="width:100%"></textarea>
			</fieldset>
			</td>
		</tr>
		...
		<tr>
			...
			<fieldset class="l2_fieldset">
				...
				<textarea name="report_taxi_in_recommendation_ckeditor" rows="3" style="width:100%"></textarea>
			</fieldset>
			</td>
		</tr>
		...
		<tr>
			...
			<fieldset class="l2_fieldset">
				...
				<div style="width:100%;" align="right"><a id="id_report_taxi_in_attach_file_btn" href="#">${lang.getStringAttachFiles()}</a></div>
				<div style="height:6px;"></div>
				<table id="id_taxi_in_attached_file_list_table" class="scroll" cellpadding="0" cellspacing="0"></table>
				<div id="id_taxi_in_attached_file_list_pager" class="scroll"></div>
			</fieldset>
			</td>
		</tr>
	</tbody>
