...
<script>
  $(function() {
   ...
   $("#id_take_off_previous_btn")
   .button()
   .click(function( event ) {
     changeTab(2);
     event.preventDefault();
   });
   $("#id_take_off_next_btn")
   .button()
   .click(function( event ) {
	 changeTab(4);
     event.preventDefault();
  ...
  function take_off_disable_hazard_selector_from_level(level){
	  ...
	  for(var cur_level=level+1 ; cur_level <= 5 ; cur_level++){
		  ...
		  var selectItem = document.getElementById('id_take_off_level_'+cur_level+'_selector');
  ...
  function take_off_enable_description_of_new_hazard(val){
	  if (val == 'y'){
		  $('#id_take_off_description_of_new_hazard').removeAttr("disabled");
	  }else if( val == 'n'){
		  $('#id_take_off_description_of_new_hazard').attr("disabled", true);
	  }
  ...
  function take_off_read_hazard_item_list_top(callback){	
		$.ajax({
			...
			success: function(msg){
				...
				var selectItem = document.getElementById('id_take_off_level_1_selector');
				//Remove all Items
				for(var count = 0 ; count < selectItem.options.length ; count++)
				{
					selectItem.options[count] = null;
  ...
  function take_off_read_hazard_item_list_children(id, level, callback){ 
		$.ajax({
			...
			success: function(msg){
				...
				Result = jQuery.trim(Result);
				var platforms = Result.split(",");
				var selectItem = document.getElementById('id_take_off_level_'+(parseInt(level, 10)+1)+'_selector');
				//Remove all Items
				for(var count = 0 ; count < selectItem.options.length ; count++)
				{
					selectItem.options[count] = null;
  ...
  {
	  var return_str = '<a id="id_take_off_seq_'+cellvalue+'_edit_hazard" href="#">Edit</a><a id="id_take_off_seq_'+cellvalue+'_delete_hazard" href="#">Delete</a>';
		return_str += '<script>';
		return_str += '$("#id_take_off_seq_'+cellvalue+'_edit_hazard").button().click(function( event ) {'
		return_str += '    	event.preventDefault();';
		return_str += '});';
		return_str += '$("#id_take_off_seq_'+cellvalue+'_delete_hazard").button().click(function( event ) {'
		return_str += '    	event.preventDefault();';
		return_str += '});';
		return_str += '</scr'+'ipt>';
	  	return return_str;
	  ...
	  jQuery("#id_take_off_hazardListTable").jqGrid({
	  	...
	  	datatype: "xml", 
	     	...
	     	colModel:[
	     	    		...
	     	    		{name:'fn'			,index:'fn'				,width:95	,align:"center", formatter:take_off_fnFormatter	}		
	     	    	],
	     	//shrinkToFit:true,
	     	//altRows:true,
	     	hoverrows:false,
	  	...
	  	loadComplete: function(){ 
	  			...
	  			//jQuery("#id_take_off_hazardListTable").setRowData(ids[i],{detail:detailHtml});				
...
<body>
<h2 class="ui-widget-header">TAXI-OFF INFORMATION</h2>
...
<tr>
	...
	<td><select id="id_take_off_level_4_selector" onchange="take_off_read_hazard_item_list_children(this.value,4);" name="method" class="hazard_item_selector">
	</select> </td>
</tr>
<tr>
	<td class="leftmost_label">Level5: </td>
	<td><select id="id_take_off_level_5_selector" name="method" class="hazard_item_selector">
	</select> </td>
</tr>
</tbody>
</table>
...
<tr>
	...
	<td> <input type="text" id="id_take_offspecific_items" class="hazard_item_text_input" /> </td>
</tr>
</tbody>
</table>
<table>
<tbody>
<tr>
	<td class="leftmost_label"></td><td align="right" width="723px;"><a id="id_take_offsave_hazard" href="#">Done</a> <a id="id_take_offmore_hazards" href="#">More Hazards?</a></td>
</tr>
</tbody>
</table>
...
<tr>
	...
	<td align="center" width="723px;" ><table id="id_take_off_hazardListTable" class="scroll" cellpadding="0" cellspacing="0"></table>
...
<tr>
	...
	<td> <input type="radio" name="new_hazard" id="take_off_new_hazard_yes" onchange="take_off_enable_description_of_new_hazard('n');" value="n" checked="checked"/></td>
	<td style="width:30px;text-align:right;">Yes</td>
	<td> <input type="radio"name="new_hazard" id="take_off_new_hazard_no" onchange="take_off_enable_description_of_new_hazard('y');" value="y"/></td>
</tr>
</tbody>
</table>
<table>
...
<tr>
	...
	<td><textarea rows="4" cols="100" id="id_take_off_description_of_new_hazard" disabled="disabled"></textarea></td>
</tr>
</tbody>
</table>
...
<tr>
	...
	<td><textarea rows="4" cols="100" id="id_take_off_narrative"></textarea></td>
</tr>
<tr>
	<td class="leftmost_label">Recommendation</td>
	<td><textarea rows="4" cols="100" id="id_take_off_recommendation"></textarea></td>
</tr>
</tbody>
</table>
<table width="100%">
